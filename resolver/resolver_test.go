...
func TestMagicResolver_Resolve_UnsupportedAsset(t *testing.T) {
	resolver := NewMagicResolver()
	...
	testCases := []struct {
		...
		{
			name:    "ethereum_unknown_asset",
			chainID: "ethereum",
			assetID: "unknown",
		},
		{
			name:    "bitcoin_eth_asset",
			chainID: "bitcoin",
			assetID: "eth", // ETH not supported on Bitcoin
		},
		{
			name:    "arbitrum_btc_asset",
			chainID: "arbitrum",
			assetID: "btc", // BTC not configured on Arbitrum
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err == nil {
				t.Fatalf("Expected error for unsupported asset %s on chain %s, but got result: %v", tc.assetID, tc.chainID, result)
			}

			expectedError := "no treasury address configured for asset " + tc.assetID + " on chain " + tc.chainID
			if err.Error() != expectedError {
				t.Errorf("Expected error message %q, got %q", expectedError, err.Error())
			}
...
func TestMagicResolver_resolveTreasury_AssetNotFound(t *testing.T) {
	resolver := NewMagicResolver()

	result, err := resolver.resolveTreasury("ethereum", "nonexistent")

	if err == nil {
		t.Fatalf("Expected error for nonexistent asset, but got result: %v", result)
	}

	expectedError := "no treasury address configured for asset nonexistent on chain ethereum"
	if err.Error() != expectedError {
		t.Errorf("Expected error message %q, got %q", expectedError, err.Error())
	}
}
