...
// BitcoinCash engine for Bitcoin Cash with CashAddr support.
type BitcoinCash struct{}

// NewBitcoinCash creates a new Bitcoin Cash engine.
func NewBitcoinCash() *BitcoinCash {
	return &BitcoinCash{}
}

// Supports returns true if this engine supports the given chain.
func (b *BitcoinCash) Supports(chain common.Chain) bool {
	return chain == common.BitcoinCash
}

// Evaluate validates a transaction against the given rule.
func (b *BitcoinCash) Evaluate(rule *types.Rule, txBytes []byte) error {
	...
	if rule.GetTarget().GetTargetType() != types.TargetType_TARGET_TYPE_UNSPECIFIED {
		return fmt.Errorf("target type must be nil for bitcoincash, got: %s", rule.GetTarget().GetTargetType().String())
	}

	tx, err := b.parseTx(txBytes)
	if err != nil {
		return fmt.Errorf("failed to parse bitcoincash transaction: %w", err)
	}
...
func (b *BitcoinCash) parseTx(txBytes []byte) (*wire.MsgTx, error) {
	tx := &wire.MsgTx{}
...
func (b *BitcoinCash) validateOutputs(rule *types.Rule, tx *wire.MsgTx) error {
...
func (b *BitcoinCash) parseConstraintName(name string) (index int, cType constraintType, err error) {
...
func (b *BitcoinCash) setConstraint(
...
func (b *BitcoinCash) validateOutputConstraintCounts(outputConstraints map[int]*outputConstraints, tx *wire.MsgTx) error {
...
func (b *BitcoinCash) validateOutputConstraints(outputConstraints map[int]*outputConstraints, tx *wire.MsgTx) error {
	for i, txOut := range tx.TxOut {
		...
		if constraints.data != nil {
			...
			if er := validateConstraint("bitcoincash", constraints.data, dataStr, compare.NewString); er != nil {
				return fmt.Errorf("output %d data validation failed: %w", i, er)
			}
		} else {
			// Address+value constraint validation using CashAddr
			outputAddress, err := bchchain.ExtractCashAddress(txOut.PkScript)
			if err != nil {
			...
			if er := validateConstraint("bitcoincash", constraints.address, outputAddress, compare.NewString); er != nil {
				return fmt.Errorf("output %d address validation failed: %w", i, er)
			}

			if er := validateConstraint("bitcoincash", constraints.value, outputAmount, compare.NewBigInt); er != nil {
