...
func TestNewTestnetSDK(t *testing.T) {
	sdk := NewTestnetSDK()
	assert.NotNil(t, sdk)
}

func TestNewHTTPRPCClient(t *testing.T) {
	endpoints := []string{"https://cosmos-rest.publicnode.com"}
...
func TestSDK_Broadcast_Success(t *testing.T) {
	...
	assert.Equal(t, "TEST_COSMOS_TX_HASH", resp.TxResponse.TxHash)
}
