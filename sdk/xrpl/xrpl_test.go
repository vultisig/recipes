...
func TestNewHTTPRPCClient(t *testing.T) {
	endpoints := []string{"https://test1.com", "https://test2.com"}
	client := NewHTTPRPCClient(endpoints)
	
	if client == nil {
		t.Fatal("NewHTTPRPCClient returned nil")
	}
	
	if len(client.endpoints) != len(endpoints) {
		t.Errorf("Expected %d endpoints, got %d", len(endpoints), len(client.endpoints))
	}
	
	for i, endpoint := range endpoints {
		if client.endpoints[i] != endpoint {
			t.Errorf("Endpoint %d mismatch: expected %s, got %s", i, endpoint, client.endpoints[i])
		}
	}
...
func TestSDK_Broadcast_NoRPCClient(t *testing.T) {
	...
	if err == nil {
		t.Error("Expected error for missing RPC client")
	}
	
	if !strings.Contains(err.Error(), "rpc client not configured") {
		t.Errorf("Expected RPC client error, got: %v", err)
	}
}
...
func TestSDK_trimLeftZeros(t *testing.T) {
	...
	result := sdk.trimLeftZeros(input)
	...
	for i, b := range expected {
		if result[i] != b {
			t.Errorf("Byte %d mismatch: expected 0x%02x, got 0x%02x", i, b, result[i])
		}
	}
	
	// Test with all zeros
	allZeros := []byte{0x00, 0x00, 0x00}
	result = sdk.trimLeftZeros(allZeros)
	if len(result) != 0 {
		t.Errorf("Expected empty result for all zeros, got %d bytes", len(result))
	}
	
	// Test with no leading zeros
	noZeros := []byte{0x12, 0x34}
	result = sdk.trimLeftZeros(noZeros)
	if len(result) != len(noZeros) {
		t.Errorf("Expected no change for input without leading zeros")
	}
}
