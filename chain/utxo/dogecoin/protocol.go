...
// DOGE implements the Protocol interface for the Dogecoin protocol
type DOGE struct{}

// ID returns the unique identifier for the DOGE protocol
func (d *DOGE) ID() string {
	return "doge"
}

// Name returns a human-readable name for the DOGE protocol
func (d *DOGE) Name() string {
	return "Dogecoin"
}

// ChainID returns the ID of the chain this protocol belongs to
func (d *DOGE) ChainID() string {
	return "dogecoin"
}

// Description returns a detailed description of the DOGE protocol
func (d *DOGE) Description() string {
	return "The native cryptocurrency of the Dogecoin blockchain, used for transactions and value transfer."
}

// Functions returns a list of available functions for this protocol
func (d *DOGE) Functions() []*types.Function {
	return []*types.Function{
		{
			ID:          "transfer",
			Name:        "Transfer DOGE",
			Description: "Transfer Dogecoin to another address",
			Parameters: []*types.FunctionParam{
				{
					Name:        "recipient",
					Type:        "address",
					Description: "The Dogecoin address of the recipient",
				},
				{
					Name:        "amount",
					Type:        "decimal",
					Description: "The amount of Dogecoin to transfer",
				},
			},
		},
	}
...
func (d *DOGE) GetFunction(id string) (*types.Function, error) {
	for _, fn := range d.Functions() {
		if fn.ID == id {
			return fn, nil
		}
	}
	return nil, fmt.Errorf("function %q not found for protocol DOGE", id)
}

func (d *DOGE) MatchFunctionCall(decodedTx types.DecodedTransaction, policyMatcher *types.PolicyFunctionMatcher) (bool, map[string]interface{}, error) {
	// Check if this is a Dogecoin transaction
	if decodedTx.ChainIdentifier() != "dogecoin" {
		return false, nil, fmt.Errorf("expected Dogecoin transaction, got %s", decodedTx.ChainIdentifier())
	}
	...
	params["amount"] = decodedTx.Value() // Amount as *big.Int in koinu
	...
	displayParams["amount"] = decodedTx.Value().String() // Amount in koinu as string
...
// NewDOGE creates a new DOGE protocol instance
func NewDOGE() types.Protocol {
	return &DOGE{}
}
